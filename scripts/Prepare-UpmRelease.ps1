...
function Add-ChangelogSection([string]$Path, [string]$TargetVersion, [string[]]$Notes)
{
    ...
    if ($null -eq $Notes -or $Notes.Count -eq 0)
    {
        $Notes = @(
            ...
            "Describe any package structure change.",
            "Describe the Unity-facing impact for users."
        )
    }
...
$package._upm | Add-Member -Force -MemberType NoteProperty -Name "changelog" -Value (Get-ChangelogSection -Path $rootChangelogPath -TargetVersion $Version)
$package | ConvertTo-Json -Depth 10 | Set-Content -Path $packageJsonPath -Encoding UTF8

Copy-Item -Path $rootReadmePath -Destination (Join-Path $PackagePath "README.md") -Force
